...
class TestingSampleGenerate:
    def __init__(self, size: int):
        ...
        self._possible_hours = [default_weeks * 2, default_weeks * 3, default_weeks * 4]

    def _generate_test_row(self) -> str:
        hours = choice(self._possible_hours)
        sample_id = self._next_sample_id
        self._next_sample_id += 1
        return f'subject_{sample_id} type_{sample_id} teacher_{sample_id} {hours} group_{sample_id}'
